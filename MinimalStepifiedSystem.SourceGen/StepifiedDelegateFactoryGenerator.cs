...
public class StepifiedDelegateFactoryGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(compilationAndProperties, (spc, data) =>
        {
            ...
            var allFactoryTypes = new HashSet<(string ns, string factoryTypeName)>();
            ...
            foreach (var item in items)
            {
                ...
                allFactoryTypes.Add((ns, uniqueFactoryName));
            ...
            // Emit a ModuleInitializer class to preserve all generated factory types for NativeAOT
            if (allFactoryTypes.Count > 0)
            {
                // Use the namespace of the first generated factory as the class's namespace
                var hubNamespace = allFactoryTypes.First().ns;
                var sbInit = new StringBuilder();
                sbInit.AppendLine("// <auto-generated/>");
                sbInit.AppendLine("using System.Runtime.CompilerServices;");
                sbInit.AppendLine("using System.Diagnostics.CodeAnalysis;");
                sbInit.AppendLine($"namespace {hubNamespace}");
                sbInit.AppendLine("{");
                sbInit.AppendLine("    public static class StepifiedFactoryModuleInitializer");
                sbInit.AppendLine("    {");
                sbInit.AppendLine("        [ModuleInitializer]");
                foreach (var (ns, factoryTypeName) in allFactoryTypes.OrderBy(x => x.ns + "." + x.factoryTypeName))
                {
                    sbInit.AppendLine($"        [DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof({ns}.{factoryTypeName}))]");
                }
                sbInit.AppendLine("        public static void Initialize() { }");
                sbInit.AppendLine("    }");
                sbInit.AppendLine("}");
                spc.AddSource("StepifiedFactoryModuleInitializer.g.cs", SourceText.From(sbInit.ToString(), Encoding.UTF8));
            }
        });
    }
}
